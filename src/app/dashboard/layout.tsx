...
import { Calendar, Users, Home, LayoutDashboard, ArrowRightLeft, Settings, UserCircle, Briefcase, Mail, CalendarDays, Search, Newspaper, Banknote, Save, CheckCircle } from 'lucide-react';
      ...
      <aside className={`${isMobileMenuOpen ? 'flex' : 'hidden'} md:flex w-full md:w-64 bg-slate-900 border-b md:border-b-0 md:border-r border-slate-800 flex-col flex-none`}>
        ...
        <div className="p-4 border-t border-slate-800 space-y-3">
          ...
          <button 
            ...
            className="w-full bg-slate-800 hover:bg-slate-700 text-slate-300 px-4 py-2 rounded-lg font-bold flex items-center justify-center gap-2 transition-colors text-sm"
          >
            {showSaved ? <><CheckCircle size={16} className="text-emerald-400" /> <span className="text-emerald-400">Tersimpan!</span></> : <><Save size={16} /> <span>Save Game</span></>}
          </button>
        </div>
      </aside>
